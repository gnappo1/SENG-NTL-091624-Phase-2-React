...
- Review the `ProjectForm` component

- Observe the edit button added to `ProjectListItem` component

- Observe the updates applied in the `App` component

- Add a `useEffect` inside the `ProjectForm` component so that the side effect will run upon `projectId` updates

#### 2. Submit the edit project form and make a PATCH request

- Inside of the `ProjectForm` component, update `handleSubmit` to include a `PATCH` request
